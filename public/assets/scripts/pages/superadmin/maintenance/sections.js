...
function ManageStudentsTable(element, students, update) {
    ...
    popup.Create().then((p) => {
      ...
      const check = ListenToForm(form, function () {
        ...
        selectedStudents.forEach((student) => {
          TABLE_LISTENER.insertItem(ALLSTUDENTS.length + 1, [
  ...
  const _Edit = (id) => {
    ...
    popup.Create().then(async (p) => {
      ...
      let selectedEmployee = await GetEmployee(
        employee.getAttribute("data-current")
      );

      ListenToForm(form, function (data) {
        popup.Remove();

        TABLE_LISTENER.updateItem(id, [
          id,
          data.student_id,
          data.type,
          data.name,
        ...
        ALLSTUDENTS = ALLSTUDENTS.map((b) => {
          if (b.id === id) {
            return {
              ...data,
              id,
              status: b.status === "current" ? "edited" : "created",
            };
          }
...
function ManageSubjectsTable(element, subjects, update) {
  ...
  const _Edit = (id) => {
    const popup = new Popup(
      "section_subjects/view_section_subject",
      { id },
      {
        backgroundDismiss: false,
      }
    ...
    popup.Create().then((p) => {
      ...
      ListenToForm(form, function (data) {
        ...
        if (SCHEDULES.length) {
          const item = Object.values(TABLE_LISTENER.getAsObject(id)).map(
            (i) => i.value
          );
          ...
          TABLE_LISTENER.updateItem(id, [
            ...
            item[1],
            item[2],
            SCHEDULES.map((sched) => sched.day).join(","),
          ]);
...
function ViewRequest(id) {
  ...
  popup.Create().then((pop) => {
    ...
    let selected_user = {
      professor_id: select.getAttribute("data-value"),
    };
    let students = [],
      subjects = [];
