...
export class FormQuestion {
  ...
  createQuestionElement() {
    ...
    div.innerHTML = `
            ...
            <div class="points-container" style="position: absolute; bottom: 10px; left: 1px; background: #ffffff; border: 1px solid #e2e8f0; border-radius: 8px; box-shadow: 0 2px 5px rgba(0, 0, 0, 0.1); padding: 6px; transition: background 0.3s;">
  ...
  getQuestionDataFromPreview() {
    ...
    return {
      ...
      choices: questionData.choices
        ? questionData.choices.map((choice) => choice.choice)
        : [],
      choices_id: questionData.choices
        ? questionData.choices.map((choice) => choice.form_question_choice_id)
        : [],
  ...
  getQuestionSummaryAnswer() {
    const questionData = this.isPreview
      ? this.getQuestionDataFromPreview()
    ...
    switch (questionData.type) {
      case "multiple-choice":
        const selectedRadio = questionElement.querySelector(
      ...
      case "checkbox":
        const selectedCheckboxes = questionElement.querySelectorAll(
        ...
        if (selectedCheckboxes.length > 0) {
          ...
          choice_id = JSON.stringify(
            Array.from(selectedCheckboxes).map((cb) => cb.dataset.choiceId)
          );
        }
        break;
      ...
      case "dropdown":
        const selectedOption =
          questionElement.querySelector("select").selectedOptions[0];
        if (selectedOption) {
      ...
      case "true-false":
        const selectedTrueFalse = questionElement.querySelector(
          'input[type="radio"]:checked'
        );
      ...
      case "matching":
        ...
        const questionAnswers = Array.from(matchingSelects).map(
          (select) => select.value
        );
  ...
  renderForAnswer() {
    ...
    switch (questionData.type) {
      case "multiple-choice":
        answerHTML = questionData.choices
          .map(
            (choice, idx) => `
                    <div class="choice-item">
      ...
      case "dropdown":
        answerHTML = `
                    <div class="choice-item">
                        <select class="choice-input" name="q${
      ...
      case "fill-blank":
        ...
        answerHTML = `
                    <div class="fill-blank-answer">
                        ${text.replace(
                          /<span class="blank-space"[^>]*data-blank-id="([^"]*)"[^>]*>[^<]*<\/span>/g,
                          (match, blankId) => `
                                ...
                                ${matchingWords
                                  .map(
                                    ...
                                    <div class="matching-word" data-word-index="${index}">
                                        ...
                                        <span class="word-letter">${String.fromCharCode(
                                          65 + index
                                        )}</span>
                                    </div>
                                `
                            ...
                            ${matchingQuestions
                              .map(
                                ...
                                <div class="matching-answer-row">
                                    <span class="answer-label">Question ${
                                      qIndex + 1
                                    } matches with:</span>
                                    <select name="match_${
                                      questionData.questionNumber
                                    }_${qIndex}" class="matching-select">
    ...
    div.innerHTML = `
            ...
            <div class="question-header">
                <div class="question-input">
                    <div class="question-title" style="border: none; background: none;">${
                      questionData.title
                    }</div>
                </div>
            </div>
  ...
  getQuestionSummary() {
    const questionNumber =
      this.element.querySelector(".question-number").textContent;
    const type = this.element.querySelector(".question-type").value;
    const imageElement = this.element.querySelector(".question-image");
    const imageUrl =
      imageElement && imageElement.src !== "" ? imageElement.src : null;
    const pointsInput = this.element.querySelector(".points-input");

    // Handle matching type
    if (type === "matching") {
      const questions = Array.from(
        this.element.querySelectorAll(".matching-input.question-input")
      )
        .map((input) => input.value.trim())
        .filter((value) => value !== "");

      const words = Array.from(
        this.element.querySelectorAll(".matching-input.word-input")
      )
        .map((input) => input.value.trim())
        .filter((value) => value !== "");
    ...
    if (type === "true-false") {
      return {
        questionNumber: parseInt(questionNumber),
        title:
          this.element.querySelector(".question-title").value ||
          "Untitled Question",
        type: type,
        choices: ["True", "False"],
        imageUrl: imageUrl,
  ...
  duplicate() {
    ...
    originalChoices.forEach((choice, index) => {
      ...
      newChoice.innerHTML = `
                ...
                <input type="text" placeholder="Choice ${
                  index + 1
                }" class="choice-input" value="${choice.value}">
                <button class="remove-choice" title="Remove choice">
                    <i data-feather="trash-2"></i>
                </button>
            `;
    ...
    if (originalImage && originalImage.src) {
      ...
      const newImageContainer = newQuestion.element.querySelector(
        ".question-image-container"
      );
      const newRemoveBtn =
        newQuestion.element.querySelector(".remove-image-btn");
  ...
  initializeMatchingType() {
    ...
    const updateNumbers = () => {
      ...
      wordsList.querySelectorAll(".matching-item").forEach((item, index) => {
        item.querySelector(".item-number").textContent = String.fromCharCode(
          65 + index
        );
      });
    };
    ...
    const createItem = (type) => {
      ...
      item.innerHTML = `
                ...
                <input type="text" class="matching-input ${type}-input" 
                       placeholder="Enter ${
                         type === "question" ? "question" : "word to match"
                       }">
...
export default class FormCreator {
  ...
  saveForm() {
    ...
    const totalPoints = mainData.questions.reduce(
    ...
    const popup = new Popup(
      `forms/add_new_form`,
      { title: obj.title, description: obj.description, totalPoints },
      {
    ...
    popup.Create().then((pop) => {
      ...
      ListenToForm(
        popup.ELEMENT.querySelector("form.form-control"),
        function (data) {
          obj
            .publishForm({ ...mainData, ...data })
            .then((res) => {
              NewNotification({
                type: "success",
...
export class FormCorrectionCreator {
  ...
  createQuestionPreview(questionData) {
    ...
    if (questionData.question_type === "true-false") {
      ...
      questionData.question_type === "checkbox"
    ) {
      choicesHtml = questionData.choices
        .map((choice, index) => {
          console.log("Processing choice:", choice); // Debug log
          const letterChoice = getLetterChoice(index);
          return `
                    <div class="choice-item">
                        <span class="choice-type-indicator" style="border-radius: ${
                          questionData.question_type === "multiple-choice"
                            ? "50%"
                            : "4px"
                        }"></span>
                        <input type="${
                          questionData.question_type === "multiple-choice"
                            ...
                            : "checkbox"
                               ...
                               value="${choice.choice}"
                               data-choice-id="${
                                 choice.form_question_choice_id
                               }"
                               class="correct-answer-input"
                               style="display: none;">
                        <label class="choice-input" for="q${
                          questionData.form_question_id
                        }_${choice.form_question_choice_id}">
      ...
      choicesHtml = `
                <div class="choice-item">
                    <select name="question_${
                      questionData.form_question_id
                    }" class="correct-answer-input choice-input">
      ...
      questionData.question_type === "short-answer" ||
      questionData.question_type === "paragraph"
    ) {
      choicesHtml = `
                <div class="text-answer-correction">
                    <div class="correction-type-selector">
                        <div class="correction-type">
                            <div class="correction-option ${
                              questionData.correction_type === "specific"
                                ? "selected"
                                : ""
                            }"
                                 data-correction-type="specific">
                                <input type="radio" 
                                       name="correction_type_${
                                         questionData.form_question_id
                                       }" 
                                       value="specific"
                                       class="correction-type-input"
                                       ${
                                         questionData.correction_type ===
                                         "specific"
                                           ? "checked"
                                           : ""
                                       }
                        ...
                        <div class="correction-type">
                            <div class="correction-option ${
                              questionData.correction_type === "keyword"
                                ? "selected"
                                : ""
                            }"
                    ...
                    <div class="correction-input-container">
                        ...
                        <div class="specific-answer-container" style="display: ${
                          questionData.correction_type === "specific"
                            ? "block"
                            : "none"
                        }">
                        ...
                        <!-- Keywords Section -->
                        <div class="keyword-container" style="display: ${
                          questionData.correction_type === "keyword"
                            ? "block"
                            : "none"
                        }">
    ...
    div.innerHTML = `
            <div class="question-header">
                ...
                <div class="question-input">
                    <div class="question-title" style="border: none; background: none;">${
                      questionData.question || "Untitled Question"
                    }</div>
                </div>
    ...
    if (
      ...
      correctionOptions.forEach((option) => {
        option.addEventListener("click", () => {
          ...
          specificContainer.style.display =
          ...
          if (type === "specific") {
            ...
            this.correctAnswers.set(questionData.form_question_id, {
              type: "keyword",
              keywords: Array.from(
                keywordsList.querySelectorAll(".keyword-tag")
      ...
      const addKeyword = () => {
        ...
        if (keyword) {
          ...
          // Add remove handler
          keywordTag
            .querySelector(".remove-keyword-btn")
  ...
  saveCorrection(data) {
    return new Promise((resolve) => {
      if (this.formData.form_correction) {
        EditRecord("form_corrections", {
          id: this.formData.form_correction.correction_id,
        ...
        AddRecord("form_corrections", { data: JSON.stringify(data) }).then(
          (res) => {
            NewNotification({
              type: "success",
              message: res.message,
      ...
      if (questionContainer) {
        if (
          ...
          const options = questionContainer.querySelectorAll(
            ".correction-type .correction-option"
          );
          ...
          if (answer.type == "specific") {
            ...
            const specificInput = questionContainer.querySelector(
              `textarea.specific-input`
            );
            if (specificInput) {
              specificInput.value =
                answer.type === "specific" ? answer.text : "";
            }
          } else {
            setTimeout(function () {
              options[1].click();
            ...
            const keywordTagsContainer =
              questionContainer.querySelector(".keywords-list");
          ...
          if (answer.length) {
            ...
            if (radioInput) {
              ...
              if (choiceItem) {
                ...
                const indicator = choiceItem.querySelector(
                  ".choice-type-indicator"
                );
                if (indicator) {
          ...
          answer.forEach((ans) => {
            const checkboxInput = questionContainer.querySelector(
              `input[value="${ans}"]`
            );
            if (checkboxInput) {
          ...
          const trueInput = questionContainer.querySelector(
            'input[value="True"]'
          );
          const falseInput = questionContainer.querySelector(
            'input[value="False"]'
          );
          ...
          if (answer === "True") {
            if (trueInput) {
              ...
              if (trueChoiceItem) {
                ...
                const trueIndicator = trueChoiceItem.querySelector(
                  ".choice-type-indicator"
                );
                if (trueIndicator) {
            ...
            if (falseInput) {
              ...
              if (falseChoiceItem) {
                ...
                const falseIndicator = falseChoiceItem.querySelector(
                  ".choice-type-indicator"
                );
                if (falseIndicator) {
          ...
          const blankInputs =
            questionContainer.querySelectorAll(`input.blank-input`);
          answer.forEach((blankAnswer) => {
            const input = Array.from(blankInputs).find(
              (input) => input.dataset.blankId === blankAnswer.blankId
            );
            if (input) {
          ...
          const matchingSelects = questionContainer.querySelectorAll(
            `select[name^="match_${questionId}"]`
          );
          for (const questionIndex in answer) {
            const select = Array.from(matchingSelects).find((select) =>
              select.name.endsWith(questionIndex)
            );
            if (select) {
  ...
  getCorrectAnswers() {
    ...
    this.formData.questions.forEach((questionData) => {
      ...
      if (questionData.question_type === "true-false") {
        ...
        const selectedOptions = questionContainer.querySelectorAll(
          `input[name="question_${questionId}"]:checked`
        );
        ...
        const keywordTags =
          questionContainer.querySelectorAll(".keyword-tag span");
...
export class FormRenderer {
    ...
    this.userAnswers.forEach((answer) => {
      ...
      switch (answer.type) {
        case "multiple-choice":
          ...
          if (radioInput) {
            radioInput.checked = true;
            const allRadios = questionElement.querySelectorAll(
              'input[type="radio"]'
            );
        ...
        case "checkbox":
          ...
          const allCheckboxes = questionElement.querySelectorAll(
            'input[type="checkbox"]'
          );
          ...
          answers.forEach((answer) => {
            const checkboxInput = questionElement.querySelector(
              `input[value="${answer}"]`
            );
            if (checkboxInput) {
              checkboxInput.checked = true;
        ...
        case "true-false":
          const trueInput = questionElement.querySelector(
            'input[value="True"]'
          );
          const falseInput = questionElement.querySelector(
            'input[value="False"]'
          );
        ...
        case "fill-blank":
          const blankInputs =
            questionElement.querySelectorAll(`input.blank-input`);
        ...
        case "matching":
          ...
          for (
            ...
            question_index < matching_aswers.length;
            question_index++
          ) {
            const select = Array.from(matchingSelects).find((select) =>
              select.name.endsWith(question_index)
            );
  ...
  initialize() {
    ...
    this.formData.questions.forEach((questionData, index) => {
      ...
      // Handle all inputs
      const inputs = renderedQuestion.querySelectorAll(
      ...
      inputs.forEach((input) => {
        ...
        if (input.type === "radio") {
          ...
          input.addEventListener("change", () => {
            const checkedBoxes =
              renderedQuestion.querySelectorAll("input:checked");
            const choiceIds = Array.from(checkedBoxes).map(
              (cb) => cb.dataset.choiceId
            );
            this.answers.set(questionData.question_id, {
              answer_type: "choices",
              answer: choiceIds,
  ...
  saveForm() {
    const formAnswers = {
      ...
      answers: Array.from(
        this.container.querySelectorAll(".question-container")
      )
        .map((questionElement) => {
...
function initializeMatchingConnections(questionNumber) {
  const container = document.querySelector(
    `#matching-canvas-${questionNumber}`
  ).parentElement;
...
function drawConnections(questionNumber) {
  ...
  selects.forEach((select, qIndex) => {
    if (select.value) {
      ...
      const questionPoint = questions[qIndex].querySelector(
        ".connection-point.right"
      );
      ...
      ctx.bezierCurveTo(
        startX + canvas.width * 0.4,
        startY, // First control point
        startX + canvas.width * 0.6,
        endY, // Second control point
        endX,
        endY // End point
      );
